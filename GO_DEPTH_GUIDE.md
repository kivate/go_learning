...
這份文件針對已了解 Go 基礎語法的入門者，深入解說三個核心觀念。
...
## 2. 集合操作：Slice 與 Map 的底層原理

### 2-0. make：初始化的關鍵函數

在學 Slice 和 Map 之前，先理解 `make`。
...
```go
make([]T, len)         // 長度 len，容量 = len
make([]T, len, cap)    // 長度 len，容量 cap（預留空間）
```

```go
s1 := make([]int, 3)      // [0 0 0]，len=3, cap=3
s2 := make([]int, 3, 10)  // [0 0 0]，len=3, cap=10
s3 := make([]int, 0, 10)  // []，len=0, cap=10（最常見：先預留，再 append）
...
```go
make(map[K]V)        // 基本初始化
make(map[K]V, hint)  // 預估 hint 個 key，減少擴容次數（非硬性限制）
```

```go
m1 := make(map[string]int)       // 空 map，可以直接寫入
m2 := make(map[string]int, 100)  // 預估會放 100 個 key，效能較好
...
```go
make(chan T)      // 無緩衝：發送與接收必須同時就緒
make(chan T, n)   // 有緩衝：緩衝大小 n，放滿才阻塞
```
...
```go
// new：分配記憶體，回傳指標，值是零值（不初始化內部結構）
p := new(int)              // *int，*p = 0，可以使用
m := new(map[string]int)   // *map，但 *m 是 nil，還不能寫入！

// make：初始化內部結構，回傳可直接使用的值（不是指標）
m := make(map[string]int)  // map，可以直接寫入
```

```text
new vs make 對比：

new(T)    → 分配 → 回傳 *T（指標），值是 T 的零值
...
> **實務上** `new` 很少用，Go 程式碼中幾乎都用 `make` 初始化 Slice/Map/Channel，其他型別則用 `&T{...}` 的複合字面值（composite literal）語法。

---

### 2-1. Slice 的底層結構

Slice 看起來像動態陣列，但底層其實是一個包含三個欄位的結構：
...
> **效能提示**：如果知道大概需要多少元素，提前用 `make([]int, 0, n)` 指定容量，可以避免多次擴容，提升效能。
...
Map 是一個雜湊表（hash table），Go runtime 在底層管理所有的雜湊計算與桶（bucket）分配，開發者不需要手動管理。
...
fmt.Println(m["cherry"])  // 0（不是 panic）
...
```go
// 危險：兩個 goroutine 同時寫 map，會 panic
m := map[string]int{}
go func() { m["a"] = 1 }()
go func() { m["b"] = 2 }()
...
// 更好的寫法，用 defer 確保解鎖
func safeWrite(mu *sync.Mutex, m map[string]int, k string, v int) {
    mu.Lock()
    defer mu.Unlock()
    m[k] = v
}
...
| 場景 | 建議方案 |
| :--- | :--- |
| 只有一個 Goroutine 存取 | 直接用 `map` |
| 多個 Goroutine，讀寫都頻繁 | `map` + `sync.Mutex` |
| 多個 Goroutine，讀多寫少 | `sync.Map` |
| 高併發寫入，效能要求高 | 分片鎖（sharded map） |
...
```go
// 定義介面：只要有 Speak() 方法，就是 Animal
type Animal interface {
    Speak() string
}

// Dog 實作了 Speak()，自動滿足 Animal 介面（不用寫 implements）
...
### 3-2-1. 編譯期驗證：Go 的「顯式宣告」慣用法

隱式實現的缺點是：少寫了某個方法，**執行時才會發現**。Go 社群有一個慣用技巧，能在**編譯時**就強制驗證介面是否實作完整：
...
Animal           → 要驗證的介面
(*Dog)(nil)      → Dog 指標的 nil 值，不分配任何記憶體，純粹用來做型別比對
```
...
// 加上這行後，編譯直接報錯，不會等到執行時才發現
var _ Animal = (*Dog)(nil)
// 錯誤訊息：
// cannot use (*Dog)(nil) (type *Dog) as type Animal:
//   *Dog does not implement Animal (missing Move method)
```
...
var _ Animal = Dog{}    // 通過：值與指標都能用值接收者的方法
var _ Animal = (*Dog)(nil) // 也通過
...
// 明確宣告：MySQLDB 必須完整實作 Database 介面
// 任何人修改 MySQLDB 時，若不小心刪除了某個方法，編譯就會提醒
...
// saveUser 不在乎是哪種資料庫，只要實作 Database 介面就行
...
// MySQL 實作
type MySQL struct{ ... }
func (m *MySQL) Insert(user User) error { ... }
func (m *MySQL) FindByID(id int) (User, error) { ... }

// PostgreSQL 實作
type PostgreSQL struct{ ... }
func (p *PostgreSQL) Insert(user User) error { ... }
func (p *PostgreSQL) FindByID(id int) (User, error) { ... }
...
> 雖然方便，但用太多 `any` 會失去型別安全的好處，應優先使用具體介面。

---

### 3-5. 型別斷言（Type Assertion）
...
型別斷言除了取出具體型別，還可以用來**呼叫介面以外的專屬方法**，有兩種常見模式。

#### 模式一：斷言成具體型別，呼叫專屬方法

介面只暴露共同方法，但某些型別可能有額外的專屬方法，斷言後才能呼叫：
...
type Dog struct{ Name string }
func (d Dog) Speak() string { return "汪" }
func (d Dog) Fetch() string { return "撿球！" } // Dog 專屬，介面沒有定義

var a Animal = Dog{Name: "小黑"}

a.Speak() // 透過介面只能呼叫 Speak()
// a.Fetch() // 編譯錯誤！介面沒有 Fetch()

// 斷言成 Dog 後才能呼叫 Fetch()
if d, ok := a.(Dog); ok {
    fmt.Println(d.Fetch()) // 撿球！
}
```

#### 模式二：斷言成另一個介面（介面升級）

檢查一個型別是否「額外」實作了某個介面，用來**選擇性地呼叫擴充方法**，這是最實用的模式：
...
type Dog struct{}
func (d Dog) Speak() string { return "汪" }
func (d Dog) Swim() string  { return "狗爬式！" } // Dog 額外實作 Swimmer
...
a.(Swimmer) → 動態檢查 a 背後的具體型別是否也滿足 Swimmer
```

#### 標準庫的實際應用

Go 標準庫大量使用這個模式，最常見的例子是 HTTP 的串流輸出：

```go
func handler(w http.ResponseWriter, r *http.Request) {
    // http.ResponseWriter 是基本介面，只有 Write / Header / WriteHeader
    // 但某些實作（如 nginx、cdn）支援 Flush() 立即推送資料
    if flusher, ok := w.(http.Flusher); ok {
        // 支援的情況下才呼叫，不支援也不會 panic
        flusher.Flush()
    }
}
```
...
```go
// 檢查 Reader 是否同時也是 WriterTo，效能更好
if wt, ok := src.(io.WriterTo); ok {
    return wt.WriteTo(dst)
}

// 檢查 Writer 是否同時也是 ReaderFrom
...
| :--- | :--- | :--- |
| `a.(Dog)` | 取出具體型別 | panic |
| `a.(Dog)` 搭配 `ok` | 安全取出具體型別 | ok = false，不 panic |
| `a.(Swimmer)` 搭配 `ok` | 介面升級，呼叫擴充方法 | ok = false，不 panic |

---

### 3-6. 型別選擇（Type Switch）
...
describe(42)               // 整數，值是 42
describe("hello")          // 字串，值是 "hello"
describe(Dog{Name: "小黑"}) // 是狗，名字是 小黑
```
...
// ReadWriter 同時要求實作 Read 和 Write
...
// File 同時滿足 Reader、Writer、ReadWriter
...
> Go 標準庫中最常見的 `io.Reader` 只有一個方法：`Read(p []byte) (n int, err error)`，這就是「小介面」的最佳典範。
